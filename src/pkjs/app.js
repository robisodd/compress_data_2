...
function send_some_text_to_pebble() {
  ...
  var data = lots_of_text.create_text();  // Generate a lot of text from external file
  ...
  jigsaw.send_to_pebble(png, successfully_complete, failed_to_send, piece_update);
}

function successfully_complete() {console.log("Successfully sent PNG to pebble");}
function failed_to_send() {console.log("Failed sending PNG to pebble");}
function piece_update() {
  console.log("PIECE Update! " + jigsaw.piece_size + " bytes sent! " +
              "Total = " + jigsaw.bytes_sent + " / " + jigsaw.size + " (" + Math.round(jigsaw.bytes_sent*100/jigsaw.size) + "%)");
}